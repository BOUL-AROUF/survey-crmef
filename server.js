...
app.get('/api/analytics', async (_req, res) => {
  try {
    ...
    const { rows: [{ c: today }] } = await pool.query(
      `SELECT COUNT(*) AS c FROM responses WHERE created_at::date = CURRENT_DATE`
    );
    const { rows: [{ c: week }] } = await pool.query(
      `SELECT COUNT(*) AS c FROM responses WHERE created_at >= NOW() - INTERVAL '7 days'`
    );
    ...
    res.json({
      ...
      q1:  await dist('q1_sexe'),
      q2:  await dist('q2_experience'),
      q3:  await dist('q3_etablissement'),
      q4:  await dist('q4_utilisation'),
      q5:  await multiDist('q5_outils'),
      q6:  await dist('q6_frequence'),
      q7:  await multiDist('q7_mode'),
      q8:  await dist('q8_comprehension'),
      q9:  await dist('q9_motivation'),
      q10: await dist('q10_autonomie'),
      q11: await dist('q11_engagement'),
      q12: await dist('q12_numerique'),
      q13: await multiDist('q13_avantages'),
      q14: await dist('q14_role'),
      q15: await multiDist('q15_difficultes'),
      q16: await dist('q16_formation'),
      q17: await multiDist('q17_type_formation'),
      q18: await multiDist('q18_ameliorations'),
      q19: await dist('q19_integration'),
      q20: await dist('q20_recommandation'),
    });
  } catch (err) {
    console.error(err);
